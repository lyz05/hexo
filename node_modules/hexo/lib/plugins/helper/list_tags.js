...
function listTagsHelper(tags, options) {
  ...
  if (style === 'list') {
    ...
    tags.forEach(tag => {
      ...
      result += `<a class="${className}-list-link" href="${this.url_for(tag.path)}${suffix}index.html">`;
    ...
    tags.forEach((tag, i) => {
      ...
      result += `<a class="${className}-link" href="${this.url_for(tag.path)}${suffix}index.html">`;
      result += transform ? transform(tag.name) : tag.name;

...
因直连的数据经过clash比较考验设备的性能，这样做能最大程度减少对国内流量的干扰和影响。避免clash挂掉对整个家庭网络造成影响。
...
plugins:
  ...
  - tag: china_ip
    ...
    args:
      files:
        - /etc/mosdns/all_cn.txt
        - /etc/mosdns/all_cn_ipv6.txt
...
wget "https://fc.home999.cc/sub?user=congcong&ctype=clashfiltertun" -O /etc/clash/config.yaml

...
---
# 前言
一直苦于看非B站视频不能下载弹幕，遂上网搜索下载非B站视频弹幕的方法，后找到一篇博客较为满意。但为了更方便的下载弹幕，而不用本地跑Python代码。萌生了将这份代码做成HTTP触发器的念头，以后只需要使用浏览器，直接调用接口，就可下载弹幕文件。方便多了！如同直接访问`comment.bilibili.com`下载B站弹幕。
...
访问[`http://fc.home999.cc`](http://fc.home999.cc)

# 使用
首先，在阿里云函数中创建一个带HTTP触发器的函数。也可以在应用中心——新建应用——Python简单示例应用中创建，这是自带的Hello World模板。
删除原有文件后，在在线编辑页中创建以下三个文件`index.html` `index.py` `redirect.html`
如下图：
![在线编辑页面](/images/基于阿里云函数实现弹幕文件解析接口/001.png)
其中
`index.html`用于显示接口的主页面。
`index.py`用于接收接口调用的请求，处理之后将生成的XML文件存入OSS中，后返回`redirect.html`，重定向到OSS上去下载。（这样做的目的是为了省流量费，香港阿里云一个月5G以下流量不收钱。也可以直接返回XML的内容）
`redirect.html`仅仅写了一段js代码用来重定向到其他网页（其实可以通过返回HTTP code：301去重定向，但是不知为何返回了301，Header中写明了location，但浏览器就是不跳转）
附代码如下：
...
def save2oss(title, xml):
    #此处需要自行修改OSS相关信息
    url = "XML/" + title + ".xml"
    auth = oss2.Auth('******')
    endpoint_internal = 'oss-**-******-internal.aliyuncs.com'
    endpoint = 'oss-**-******.aliyuncs.com'
    bucketName = '*******'
    bucket = oss2.Bucket(auth, endpoint, bucketName)
    result = bucket.put_object(url, xml)
    print('Upload URL:{}\tHTTP status: {}'.format(url, result.status))
    bucket = oss2.Bucket(auth, endpoint, bucketName)
    return bucket.sign_url('GET', url, 120)
...
def handler(environ, start_response):
    ...
    if 'QUERY_STRING' not in environ:
        ...
        url = unquote(query_string.split('=')[1], 'utf-8')
        returl = build_response(url)
        if returl is not None:
            with open("redirect.html", "r", encoding="utf-8") as f:
                ret = f.read()
            ret = ret.replace("http://example.html", returl)
        else:
            ret = "不支持的视频网址"
        return [ret.encode('utf8')]
...
charset=gb2312">
	<meta http-equiv="refresh" content="0.05;url=http://example.html">
	<title></title>
</head>

<body>http://example.html</body>
